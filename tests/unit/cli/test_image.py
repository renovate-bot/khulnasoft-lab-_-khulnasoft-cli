import pytest
from khulnasoftcli.cli import image
...
def response(monkeypatch):
    ...
    def apply(success=True, httpcode=200, has_error=None):
        ...
        monkeypatch.setattr(image.khulnasoftcli.clients.apiexternal, "query_image", patch)
...
class TestDeleteImage:
    def test_deleted_pre_v080(self, monkeypatch, response):
        ...
        monkeypatch.setattr(image, "config", {"jsonmode": False})
        monkeypatch.setattr(
            image.khulnasoftcli.clients.apiexternal,
            "delete_image",
            lambda *a, **kw: {
                "success": True,
                "httpcode": 200,
