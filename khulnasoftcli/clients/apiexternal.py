...
import khulnasoftcli.clients.common
...
def set_account_header(config):
    ...
    if config["as_account"] is not None:
        header_overrides["x-khulnasoft-account"] = config["as_account"]
    else:
        header_overrides.pop("x-khulnasoft-account", None)
...
def get_base_routes(config):
    ...
    try:
        ...
        ret = khulnasoftcli.clients.common.make_client_result(r, raw=False)
...
def detect_api_version(config):
    ...
    # contruct candidate URLs for finding the khulnasoft-engine swagger.json document, supporting indirection through proxies and base khulnasoft-engine service
    urls = []
    try:
...
def import_image(config, khulnasoft_data=[]):
    userId = config["user"]
    password = config["pass"]
    base_url = config["url"]
...
def get_subscription(config, subscription_type=None, subscription_key=None):
    ...
    return khulnasoftcli.clients.common.make_client_result(r, raw=False)
...
def add_transition_rule(
    ...
    :param tag_versions_newer: Number of newer digest mappings for the tag in the khulnasoft db
