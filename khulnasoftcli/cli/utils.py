...
def setup_config(cli_opts):
    ...
    for e in [
        ...
        "KHULNASOFT_CLI_PASS",
        "KHULNASOFT_CLI_URL",
        "KHULNASOFT_CLI_HUB_URL",
        "KHULNASOFT_CLI_API_VERSION",
        "KHULNASOFT_CLI_SSL_VERIFY",
        "KHULNASOFT_CLI_JSON",
        "KHULNASOFT_CLI_DEBUG",
        "KHULNASOFT_CLI_ACCOUNT",
        "KHULNASOFT_CLI_CONFIG",
    ]:
        if e in os.environ:
    ...
    try:
        if "KHULNASOFT_CLI_CONFIG" in settings:
            credential_file = settings["KHULNASOFT_CLI_CONFIG"]
        else:
            home = os.path.expanduser("~")
            credential_file = os.path.join(home, ".khulnasoft", "credentials.yaml")
    ...
    if cli_opts["config"]:
        settings["KHULNASOFT_CLI_CONFIG"] = cli_opts["config"]
    if cli_opts["u"]:
        settings["KHULNASOFT_CLI_USER"] = cli_opts["u"]

    if cli_opts["p"]:
        settings["KHULNASOFT_CLI_PASS"] = cli_opts["p"]

    if cli_opts["url"]:
        settings["KHULNASOFT_CLI_URL"] = cli_opts["url"]

    if cli_opts["hub-url"]:
        settings["KHULNASOFT_CLI_HUB_URL"] = cli_opts["hub-url"]

    if cli_opts["api-version"]:
        settings["KHULNASOFT_CLI_API_VERSION"] = cli_opts["api-version"]

    if cli_opts["insecure"]:
        settings["KHULNASOFT_CLI_SSL_VERIFY"] = "n"

    if cli_opts["json"]:
        settings["KHULNASOFT_CLI_JSON"] = "y"

    if cli_opts["debug"]:
        settings["KHULNASOFT_CLI_DEBUG"] = "y"

    if cli_opts.get("as_account") is not None:
        settings["KHULNASOFT_CLI_ACCOUNT"] = cli_opts["as_account"]

    if "KHULNASOFT_CLI_CONFIG" in settings:
        ret["config"] = settings["KHULNASOFT_CLI_CONFIG"]
    if "KHULNASOFT_CLI_USER" in settings:
        ret["user"] = settings["KHULNASOFT_CLI_USER"]
    if "KHULNASOFT_CLI_PASS" in settings:
        ret["pass"] = settings["KHULNASOFT_CLI_PASS"]
    if "KHULNASOFT_CLI_URL" in settings:
        ret["url"] = settings["KHULNASOFT_CLI_URL"]
    if "KHULNASOFT_CLI_HUB_URL" in settings:
        ret["hub-url"] = settings["KHULNASOFT_CLI_HUB_URL"]

    if "KHULNASOFT_CLI_API_VERSION" in settings:
        ret["api-version"] = settings["KHULNASOFT_CLI_API_VERSION"]
    if "KHULNASOFT_CLI_SSL_VERIFY" in settings:
        if settings["KHULNASOFT_CLI_SSL_VERIFY"].lower() == "n":
            ret["ssl_verify"] = False
    if "KHULNASOFT_CLI_JSON" in settings:
        if settings["KHULNASOFT_CLI_JSON"].lower() == "y":
            ret["jsonmode"] = True
    if "KHULNASOFT_CLI_DEBUG" in settings:
        if settings["KHULNASOFT_CLI_DEBUG"].lower() == "y":
            ret["debug"] = True
    if "KHULNASOFT_CLI_ACCOUNT" in settings:
        ret["as_account"] = settings["KHULNASOFT_CLI_ACCOUNT"]

    return ret


def doexit(ecode):
    if not os.environ.get("KHULNASOFT_CLI_NO_FDS_CLEANUP"):
        try:
            sys.stdout.close()
        except Exception:
            pass
...
def format_error_output(config, op, params, payload):
    ...
    if op in ["account_delete"]:
        if "Invalid account state change requested" in errdata.get("message", ""):
            obuf = (
                obuf
                + "\nNOTE: accounts must be disabled (khulnasoft-cli account disable <account>) in order to be deleted\n"
            )
...
def check_access(config):
    # test the endpoint
    try:
        rc = khulnasoftcli.clients.apiexternal.get_base_routes(config)
        if not rc["success"]:
            raise Exception(json.dumps(rc["error"], sort_keys=True))
    except Exception as err:
        if config["debug"]:
            raise Exception(
                "could not access khulnasoft service (user="
                + str(config["user"])
                + " url="
                + str(config["url"])
                + "): "
...
def discover_inputimage(config, input_string):
    ...
    ret_type = "tag"
    try:
        ret = khulnasoftcli.clients.apiexternal.get_image(config, tag=input_string)
        if ret["success"]:
