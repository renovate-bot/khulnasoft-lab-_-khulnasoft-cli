...
@click.group(name="enterprise", short_help="Enterprise Khulnasoft operations")
@click.pass_obj
...
@enterprise.group(
    name="corrections",
    short_help="Enterprise Khulnasoft False-Positive Management Corrections",
)
@click.pass_obj
...
def add_correction(match, package_type, replace):
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.enterprise_add_correction(
            config, correction
        )
        if ret:
            if ret["success"]:
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "add_correction", {}, err)
        )
        if not error_code:
            error_code = 2
    khulnasoftcli.cli.utils.doexit(error_code)
...
def get_correction(correction_id):
    ...
    error_code = 0
    try:
        ret = khulnasoftcli.clients.apiexternal.enterprise_get_correction(
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "get_correction", {}, err)
        )
        if not error_code:
            error_code = 2
    khulnasoftcli.cli.utils.doexit(error_code)
...
def list_corrections():
    error_code = 0
    try:
        ret = khulnasoftcli.clients.apiexternal.enterprise_list_corrections(config)
        if ret:
            if ret["success"]:
                print(
                    khulnasoftcli.cli.utils.format_output(
...
def delete_correction(correction_id):
    ...
    error_code = 0
    try:
        ret = khulnasoftcli.clients.apiexternal.enterprise_delete_correction(
            config, correction_id
        )
        if ret:
            if ret["success"]:
