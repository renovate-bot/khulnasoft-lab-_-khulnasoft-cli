...
def event(ctx_config):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "event", {}, err))
        sys.exit(2)
...
def list(
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.list_events(
            config,
            since=since,
            before=before,
            level=level,
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "event_list", {}, err))
        if not ecode:
            ecode = 2
...
def get(event_id):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_event(config, event_id=event_id)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "event_get", {}, err))
        if not ecode:
            ecode = 2
...
def delete(since=None, before=None, dontask=False, event_id=None, all=False):
    ...
    try:
        if event_id:
            ret = khulnasoftcli.clients.apiexternal.delete_event(config, event_id=event_id)
        else:
            if all:
                if dontask:
                    answer = "y"
            ...
            if "y" == answer.lower():
                ret = khulnasoftcli.clients.apiexternal.delete_events(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "event_delete", {}, err))
        if not ecode:
            ecode = 2
