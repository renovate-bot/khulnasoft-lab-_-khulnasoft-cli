...
def image(ctx_config):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image", {}, err))
        sys.exit(2)
...
def wait(input_image, timeout, interval):
    ...
    try:
        itype = khulnasoftcli.cli.utils.discover_inputimage_format(config, input_image)
        image = input_image
        ...
        while timeout < 0 or time.time() - t1 < timeout:
            ...
            if itype == "tag":
                ret = khulnasoftcli.clients.apiexternal.get_image(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_get", {}, err))
        if not ecode:
            ecode = 2
...
def add(input_image, force, dockerfile, annotation, noautosubscribe):
    ...
    try:
        ...
        if itype == "tag":
            ...
            ret = khulnasoftcli.clients.apiexternal.add_image(
                config,
                tag=input_image,
                force=force,
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_add", {}, err))
        if not ecode:
            ecode = 2
...
@image.command(name="import", short_help="Import an image from khulnasoft scanner export")
@click.option(
...
def import_image(infile):
    ...
    try:
        with open(infile, "r") as FH:
            khulnasoft_data = json.loads(FH.read())

        ret = khulnasoftcli.clients.apiexternal.import_image(
            config, khulnasoft_data=khulnasoft_data
        )
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_import", {}, err))
        if not ecode:
            ecode = 2
...
def imagelist(full, show_all):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_images(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_list", {}, err))
        if not ecode:
            ecode = 2
...
def query_content(input_image, content_type):
    ...
    available for an image, run the following command:
        $ khulnasoft-cli image content <input_image>
    """
    ecode = 0
    ...
    try:
        ...
        if not imageDigest:
            ...
            ret = khulnasoftcli.clients.apiexternal.query_image(
                config,
            ...
            if ret:
                if ret["success"]:
                    output = khulnasoftcli.cli.utils.format_output(
                        config,
                        "image_content",
                        {"query_type": content_type},
                        ret["payload"],
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "image_content", {}, err)
        )
        if not ecode:
            ecode = 2
...
def query_metadata(input_image, metadata_type):
    ...
    try:
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "image_metadata", {}, err)
        )
        if not ecode:
            ecode = 2
...
def query_vuln(input_image, vuln_type, vendor_only):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_vuln", {}, err))
        if not ecode:
            ecode = 2
    khulnasoftcli.cli.utils.doexit(ecode)
        ...
        try:
            ...
            for image in ret["payload"]:
                if image["imageDigest"]:
                    ret = khulnasoftcli.clients.apiexternal.delete_image(
