...
def registry(ctx_config):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "registry", {}, err))
        sys.exit(2)
...
def add(
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.add_registry(
            config,
            registry=registry,
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "registry_add", {}, err))
        if not ecode:
            ecode = 2
...
def upd(
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.update_registry(
            config,
            registry=registry,
            registry_user=registry_user,
            registry_pass=registry_pass,
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "registry_update", {}, err)
        )
        if not ecode:
            ecode = 2
    khulnasoftcli.cli.utils.doexit(ecode)
...
def delete(registry):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.delete_registry(config, registry)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "registry_delete", {}, err)
        )
        if not ecode:
            ecode = 2
    khulnasoftcli.cli.utils.doexit(ecode)
...
def registrylist():
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_registry(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "registry_list", {}, err)
        )
        if not ecode:
            ecode = 2
    khulnasoftcli.cli.utils.doexit(ecode)
...
def get(registry):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_registry(config, registry=registry)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "registry_get", {}, err))
        if not ecode:
            ecode = 2
    khulnasoftcli.cli.utils.doexit(ecode)
