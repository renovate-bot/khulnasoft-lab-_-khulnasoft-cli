...
def account(ctx_config):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "account", {}, err))
        sys.exit(2)

    try:
        ret = khulnasoftcli.clients.apiexternal.get_account(config)
        if ret["success"]:
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_user(config)
        if ret["success"]:
...
def get_current_user():
    global whoami
    ecode = 0
    print(khulnasoftcli.cli.utils.format_output(config, "account_whoami", {}, whoami))
...
def add(account_name, email):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.add_account(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "account_add", {}, err))
        if not ecode:
            ecode = 2
...
def get(account_name):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_account(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "account_get", {}, err))
        if not ecode:
            ecode = 2
...
def list_accounts():
    ...
    ecode = 0
    try:
        ret = khulnasoftcli.clients.apiexternal.list_accounts(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "account_list", {}, err))
        if not ecode:
            ecode = 2
...
def delete(account_name, dontask):
    ...
    if answer.lower() == "y":
        try:
            ret = khulnasoftcli.clients.apiexternal.del_account(
                config, account_name=account_name
            )
            ecode = khulnasoftcli.cli.utils.get_ecode(ret)
            if ret["success"]:
...
def enable(account_name):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.enable_account(
            config, account_name=account_name
        )
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "account_enable", {}, err)
        )
        if not ecode:
            ecode = 2
...
def disable(account_name):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.disable_account(
            config, account_name=account_name
        )
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "account_disable", {}, err)
        )
        if not ecode:
            ecode = 2
...
def user_add(user_name, user_password, account):
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.add_user(
            config,
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "user_add", {}, err))
        if not ecode:
            ecode = 2
...
def user_delete(user_name, account):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.del_user(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "user_delete", {}, err))
        if not ecode:
            ecode = 2
...
def user_get(user_name, account):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_user(
            config, account_name=account, user_name=user_name
        )
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "user_get", {}, err))
        if not ecode:
            ecode = 2
...
def user_list(account):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.list_users(config, account_name=account)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "user_list", {}, err))
        if not ecode:
            ecode = 2
...
def user_setpassword(user_password, username, account):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.update_user_password(
            config,
            account_name=account,
            user_name=username,
            user_password=user_password,
