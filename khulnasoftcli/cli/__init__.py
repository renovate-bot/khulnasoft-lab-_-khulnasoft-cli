...
import khulnasoftcli.clients  # noqa


@click.group(context_settings=dict(help_option_names=["-h", "--help", "help"]))
@click.option(
    "--config", help="Set the location of the khulnasoft-cli yaml configuration file"
)
@click.option("--debug", is_flag=True, help="Debug output to stderr")
@click.option("--u", help="Username (or use environment variable KHULNASOFT_CLI_USER)")
@click.option("--p", help="Password (or use environment variable KHULNASOFT_CLI_PASS)")
@click.option("--url", help="Service URL (or use environment variable KHULNASOFT_CLI_URL)")
@click.option(
    "--hub-url",
    help="KhulnaSoft Hub URL (or use environment variable KHULNASOFT_CLI_HUB_URL)",
)
@click.option(
    "--api-version",
...
@click.option(
    "--insecure",
    is_flag=True,
    help="Skip SSL cert checks (or use environment variable KHULNASOFT_CLI_SSL_VERIFY=<y/n>)",
)
