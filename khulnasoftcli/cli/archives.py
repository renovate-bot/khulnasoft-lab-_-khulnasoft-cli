...
def image_restore(image_digest):
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.restore_archived_image(
            config, image_digest
        )
...
def image_add(image_digests):
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.archive_analyses(config, image_digests)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
...
def image_get(digest):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_archived_analysis(config, digest)
...
def list_archived_analyses():
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.list_archived_analyses(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
...
def image_delete(digest, force):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.delete_archived_analysis(config, digest)
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "image_delete", {}, err))
        if not ecode:
            ecode = 2
...
def rule_add(
    ...
    if max_images_per_account and not is_global:
        ...
        khulnasoftcli.cli.utils.doexit(2)
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.add_transition_rule(
            config,
            days_old,
            tag_versions_newer,
            registry_selector,
...
def rule_get(rule_id):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.get_transition_rule(config, rule_id)
...
def list_transition_rules():
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.list_transition_rules(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
...
def rule_delete(rule_id):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.delete_transition_rule(config, rule_id)
