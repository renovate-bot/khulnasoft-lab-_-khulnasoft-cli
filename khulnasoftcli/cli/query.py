...
def query(ctx_config):
    ...
    try:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "query", {}, err))
        sys.exit(2)
...
def images_by_vulnerability(
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.query_images_by_vulnerability(
            config,
            vulnerability_id,
            namespace=namespace,
...
def images_by_package(name, version, package_type):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.query_images_by_package(
