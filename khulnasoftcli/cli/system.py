...
def system(ctx_config, ctx):
    ...
    if ctx.invoked_subcommand not in ["wait"]:
        try:
            ...
            print(khulnasoftcli.cli.utils.format_error_output(config, "system", {}, err))
            sys.exit(2)


@system.command(name="status", short_help="Check current khulnasoft-engine system status")
def status():
    ecode = 0

    try:
        ret = khulnasoftcli.clients.apiexternal.system_status(config)
        ...
        print(
            khulnasoftcli.cli.utils.format_error_output(config, "system_status", {}, err)
        )
        if not ecode:
            ecode = 2
...
@system.command(
    name="errorcodes",
    short_help="Describe available khulnasoft system error code names and descriptions",
)
def describe_errorcodes():
    ecode = 0

    try:
        ret = khulnasoftcli.clients.apiexternal.describe_error_codes(config)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
...
@system.command(
    name="wait",
    short_help="Blocking operation that will return when khulnasoft-engine is available and ready",
)
@click.option(
    "--timeout",
    type=float,
...
@click.option(
    ...
    help='Wait for the specified CSV list of khulnasoft-engine services to have at least one service reporting as available (default="catalog,apiext,policy_engine,simplequeue,analyzer")',
)
def wait(timeout, interval, feedsready, servicesready):
    ...
    try:
        sys.stderr.write(
            "Starting checks to wait for khulnasoft-engine to be available timeout={} interval={}\n".format(
                timeout, interval
            )
        )
        ts = time.time()
        while timeout < 0 or time.time() - ts < timeout:
            _logger.debug(
                "Checking API availability for khulnasoft-engine URL (%s)", config["url"]
            )
            # FIXME this can still break when formatting
            sys.stderr.write(
                "API availability: Checking khulnasoft-engine URL ({})...\n".format(
                    config["url"]
                )
            )
            try:
        ...
        while timeout < 0 or time.time() - ts < timeout:
            ...
            _logger.debug(
                "Checking service set availability for khulnasoft-engine URL (%s)",
                config["url"],
            )
            # FIXME this can still break when formatting
            sys.stderr.write(
            ...
            try:
                ...
                if ret["success"]:
                    ...
                    raise Exception(
                        "service responded with 401 Unauthorized - please check khulnasoft-engine credentials and try again"
                    )
            except Exception as err:
        ...
        if feedsready:
            ...
            while timeout < 0 or time.time() - ts < timeout:
                _logger.debug(
                    "Checking feed sync status for khulnasoft-engine URL (%s)",
                    config["url"],
                )
                ...
                try:
                    ret = khulnasoftcli.clients.apiexternal.system_feeds_list(config)
                    if ret["success"]:
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "system_wait", {}, err))
        if not ecode:
            ecode = 2
...
@system.command(
    name="del", short_help="Delete a non-active service from khulnasoft-engine"
)
...
def delete(host_id, servicename):
    ...
    try:
        ret = khulnasoftcli.clients.apiexternal.delete_system_service(
...
def feedsync(flush):
    ...
    try:
        ...
        try:
            print(
                "\nWARNING: This operation should not normally need to be performed except when the khulnasoft-engine operator is certain that it is required - the operation will take a long time (hours) to complete, and there may be an impact on khulnasoft-engine performance during the re-sync/flush.\n"
            )
            try:
                input = raw_input
            except NameError:
        ...
        if "y" == answer.lower():
            ret = khulnasoftcli.clients.apiexternal.system_feeds_sync(config, flush)
            ecode = khulnasoftcli.cli.utils.get_ecode(ret)
...
def toggle_enabled(feed, group=None, enable=None, disable=None):
    ...
    try:
        ...
        if group:
            ret = khulnasoftcli.clients.apiexternal.system_feed_group_enable_toggle(
                config, feed, group, enabled=enabled
            )
            ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        else:
            ret = khulnasoftcli.clients.apiexternal.system_feed_enable_toggle(
...
def delete_data(feed, group=None):
    ...
    try:
        if group:
            ret = khulnasoftcli.clients.apiexternal.system_feed_group_delete(
                config, feed, group
            )
            ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        else:
            ret = khulnasoftcli.clients.apiexternal.system_feed_delete(config, feed)
            ecode = khulnasoftcli.cli.utils.get_ecode(ret)
...
def test_webhook(webhook_type, ntype):
    """
    Call the Test Webhook Endpoint on KhulnaSoft Engine
    ...
    try:
        ...
        ret = khulnasoftcli.clients.apiexternal.test_webhook(config, webhook_type, ntype)
        ecode = khulnasoftcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                khulnasoftcli.cli.utils.format_output(
        ...
        print(khulnasoftcli.cli.utils.format_error_output(config, "test_webhook", {}, err))
        if not ecode:
            ecode = 2
